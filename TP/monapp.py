...
def sauvegarder_inventaire(self, id_personnage):
    cursor = cnx.cursor(dictionary=True)
    # MON INVENTAIRE
    obj = self.plainTextEdit_objets_inv.toPlainText()
    repas = self.plainTextEdit_repas_inv.toPlainText()
    objspe = self.plainTextEdit_objspe_inv.toPlainText()
    bourse = self.spinBox_bourse.value()

    # INSERT INTO
    query = f'INSERT INTO sac_a_dos VALUES (DEFAULT, "{obj}", "{repas}", "{objspe}", "{bourse}", "{id_personnage}");'
    try:
...
def nouvelle_sauvegarde(self):
    ...
    sauvegarder_inventaire(self, id_personnage)
...
def insert_perso_kai(personnage_id, id_kai, notes):
    cursor = cnx.cursor(dictionary=True)
    # SQL
    
    query = f'INSERT INTO personnage_kai VALUES (DEFAULT, {id_kai}, {personnage_id}, "{notes}");'
    try:
        cursor.execute(query)
        cnx.commit()
    except Exception as e:
...
## TO DO ##
###########
def remplir_inventaire(self, id_sac):
    # PERSONNAGE
    ...
    # SQL
    cursor = cnx.cursor(dictionary=True)
    query = f"SELECT objets, repas, objets_speciaux, bourse FROM sac_a_dos WHERE id_personnage = {id_personnage} AND id = {id_sac} LIMIT 1;"
    cursor.execute(query)
...
def remplir_endurance_habilete(self):
    ...
    # REMPLIR
    self.spinHab.setValue(resultats['habilete'])
    self.spinEnd.setValue(resultats['endurance'])
...
def retourner_parties_personnage(self):
    # PERSONNAGE ACTIF
    nom_personnage_actif = trouver_personnage(self)
    id_personnage_actif = trouver_id_personnage(self, nom_personnage_actif)

    cursor = cnx.cursor(dictionary=True)
    query = f"SELECT id FROM partie WHERE id_personnage = {id_personnage_actif};"
    cursor.execute(query)
    resultats = [item['id'] for item in cursor.fetchall()]
    return resultats

def retourner_sacs_personnage(self):
    # PERSONNAGE ACTIF
    nom_personnage_actif = trouver_personnage(self)
    id_personnage_actif = trouver_id_personnage(self, nom_personnage_actif)

    cursor = cnx.cursor(dictionary=True)
    query = f"SELECT id FROM sac_a_dos WHERE id_personnage = {id_personnage_actif};"
    cursor.execute(query)
    resultats = [item['id'] for item in cursor.fetchall()]
    return resultats

def update_habilete_endurance(self):
    # PERSONNAGE ACTIF
    nom_personnage_actif = trouver_personnage(self)
    id_personnage_actif = trouver_id_personnage(self, nom_personnage_actif)
    nouvelle_habilete = self.spinHab.value()
    nouvelle_endurance = self.spinEnd.value()

    #updater le tout
    cursor = cnx.cursor(dictionary=True)
    query = f"UPDATE personnage SET habilete = '{nouvelle_habilete}', endurance = '{nouvelle_endurance}' WHERE id = '{id_personnage_actif}';"
    try:
        cursor.execute(query)
        cnx.commit()
    except Exception as e:
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def charger_sauvegarde(self):
        ...
        #Mettre chaque ID de "partie" dans un tableau
        les_parties = retourner_parties_personnage(self)
        #Mettre chaque ID de "sac" dans un tableau
        les_sacs = retourner_sacs_personnage(self)
        #boucle qui trouve la position de "id_partie" dans le tableau de partie
        x = 0
        position = 0
        for element in les_parties: 
            element_string = str(element)
            if(element_string == id_partie):    
                position = x
            else:
                x += 1
        #on trouve l'ID associé à la position dans le tableau de sac
        id_sac = les_sacs[position] # Se rappeler que id_sac commence à 1 et les_sacs commencent à 0
        #on load SAC
        remplir_inventaire(self, id_sac)
    ...
    def ecraser_sauvegarde(self):
        ...
        #updater le tout
        cursor = cnx.cursor(dictionary=True)
        query = f"DELETE FROM partie WHERE id = '{id_save}';"
        try:
            cursor.execute(query)
            cnx.commit()
            ecraser_une_sauvegarde(self)
    ...
    def sauvegarder_etat_partie(self):
        nouvelle_sauvegarde(self)
        montrer_les_sauvegardes(self)
        update_habilete_endurance(self)
        ecraser_une_sauvegarde(self)
    ...
    def creer_personnage(self):
        ...
        disciplines_notes = [self.note_discipline1.toPlainText(), self.note_discipline2.toPlainText(), self.note_discipline3.toPlainText(), self.note_discipline4.toPlainText(), self.note_discipline5.toPlainText()]
        ...
        if(valide == True):
            ...
            for element in disciplines:
                ...
                note = disciplines_notes[i]
                if(note == ""):
                    note = "Aucune note"
                insert_perso_kai(id_personnage, element, note)
                i += 1
